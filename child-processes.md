...
Node 通过`child_process` 模块提供了三向`popen(3)`数据流。
...
注意，`exit` 事件有可能在错误发生后调用，也由可能不调用，所以如果你监听这两个事件来触发函数，记得预防函数会被调用2次。
...
这个事件在子进程结束的时候触发。如果子进程正常终止，则 `code` 为最终的退出代码，否则为 `null` 。如果是由`signal` 引起的终止，则 `signal` 为字符串，否则为 `null`.

注意子进程的 stdio 流可能仍为开启模式.
...
这个事件在子进程里所有 stdio 流都关闭的时候触发。  要和 'exit' 区分开, 因为多进程可以共享一个 stdio 流.

### Event: 'disconnect'

这个事件父进程或子进程中调用`.disconnect()` 方法后触发。 断开后不会在互发消息，并且 `.connected`属性值为 false.
...
子进程的管道数组和[spawn](#child_process_child_process_spawn_command_args_options)里[stdio](#child_process_options_stdio) 里设置为 `'pipe'` 的内容相对应 (#child_process_child_process_spawn_command_args_options)的次序相对应。  
  
注意，流[0-2]也能分别用ChildProcess.stdin,ChildProcess.stdout, 和 ChildProcess.stderrNote来表示.

在下面的例子里，只有子进程的 fd `1` 设置为 pipe管道，所以只有父进程的 `child.stdio[1]` 是流（stream）, 数组里其他值为`null`.
...
发送信号给子进程。如果没有参数，子进程将会发送 `'SIGTERM'` ， 参见 `signal(7)` 里的可用的信号列表。
...
当信号无法传递的时候会触发`'error'`事件。给已经终止的进程发送信号不会触发`'error'`事件，但是可以能引起不可预知的后果: 因为有可能 PID (进程 ID) 已经重新分配给其他进程,  信号就会被发送到新的进程里，这样会引发的什么样的事情无法想象。
...
子进程的代码里的 `process` 对象拥有 `send()` 方法，当它接通过信道接收到信息的时候会触发，并返回对象。  
  
注意，在父进程和子进程里 `send()` 是同步的，不要用来发送大块的数据（可以用管道来代替，参见[`child_process.spawn`](#child_process_child_process_spawn_command_args_options)).

不过发送 `{cmd: 'NODE_foo'}` 消息是特殊情况. 所有包含 `NODE_` 前缀的消息都不会被触发，因为它们是 node 的内部的核心消息，它们会在 `internalMessage` 事件里触发，尽量避免使用这个特性。

`child.send()` 里的 `sendHandle` 属性用来发送 TCP 服务或 socket 对象给其他的进程，子进程会在第二个参数接收到对象给 `message` 对象。

如果消息不能发出会触发 `'error'` 事件，比如子进程已经退出。
#### 例子: 发送 server 对象
...
以下是发送 socket 对象的例子. 他将会创建2个子线程，并且同时处理连接，一个通过远程地址 `74.125.127.100`作为 VIP 发送到一个特殊的子进程，另外一个发送到正常进程。
...
注意，当 socket 发送给子进程后，如果这个socket被销毁，父进程不再对它保持跟踪，相应的 `.connections` 属性会变为 `null`。这种情况下，不建议使用 `.maxConnections` 。

### child.disconnect()

关闭父子进程间的所有 IPC 通道，能让子进程优雅的退出。调用这个方法后，父子进程里的`.connected` 标志会变为 `false`，之后不能再发送消息。

当进程里不再有需要处理的消息的时候，将会触发 'disconnect' 事件。  

注意，在子进程有 IPC 通道的情况下（如 `fork()` ），也可以调用 `process.disconnect()` 来关闭它。
...
从创建的子进程里使用 `cwd` 指定工作目录，如果没有指定，默认继承自当前的工作目录。

使用 `env` 来指定环境变量，这样对新进程可见。默认是 `process.env`。
...
1. `'pipe'` - 创建在父进程和子进程间创建一个 pipe。  管道的父进程端以 `child_process` 的属性形式暴露给父进程，例如 `ChildProcess.stdio[fd]` 。  为 fds 0 - 2 创建的管道也可以通过 ChildProcess.stdin, ChildProcess.stdout 和 ChildProcess.stderr 来独立的访问。

2. `'ipc'` - 在父进程和子进程间创建一个 IPC 通道来传递消息，文件描述符。 一个子进程最多只有**1个** IPC stdio 文件标识。设置这个选项会激活 ChildProcess.send() 方法。如果子进程向此文件标识写入 JSON 消息，则会触发 ChildProcess.on('message') 。如果子进程是 Node.js 程序，那么 IPC 通道会激活process.send() 和 process.on('message')。

3. `'ignore'` - 在子进程里不要设置这个文件标识，注意，Node 总会为其 spawn 的进程打开 fd 0-2。如果任何一个被ignored，node 将会打开`/dev/null` 并赋给子进程的 fd。
   
4. `Stream` 对象 - 共享一个tty, file, socket, 或 a pipe 可读或可写流给子进程。  该流底层（underlying）的文件标识在子进程中被复制给stdio数组索引对应的文件标识（fd）
...
如果设置`detached` 选项, 子进程将会被作为新进程组的leader，这使得子进程可以在父进程退出后继续运行。

缺省情况，父进程会等 detached 的子进程推出。要阻止父进程等待一个特定的子进程，调用 child.unref() 方法，则父进程的事件循环引用计数中将不会包含这个子进程。
...
当使用  `detached`  选项来启动一个长时间运行的进程，进程不会在后台保持运行，除非他提供了一个 不连接到父进程的`stdio` 。如果继承了父进程的  `stdio`，则子进程会继续控制终端。
...
值为 `null`。 如果失败, `error` 变为 `Error` 的实例，并且 `error.code`
...
如果 `timeout` 大于 0, 子进程运行时间超过 `timeout` 的时候会被终止。 `killSignal` (默认: `'SIGTERM'`) 能杀死子进程。 `maxBuffer` 设定了 stdout 或 stderr 的最大数据量，如果子进程的数量量超过了，将会被杀死。
...
* `options` {Object}
  ...
  * `execArgv` {Array} 执行参数
    (默认: `process.execArgv`)
  * `silent` {Boolean} 如果是 true ，子进程将会用父进程的 stdin, stdout, and stderr ，否则，将会继承自父进程, 更多细节，参见 `spawn()` 里的 `stdio` 参数里的 "pipe" 和 "inherit" 选项(默认 false)
  * `uid` {Number} 设置进程里的用户标识. (见 setuid(2).)
  * `gid` {Number} 设置进程里的群组标识. (见 setgid(2).)
* Return: ChildProcess 对象
...
这些 Nodes 是全新的 V8 实例化，假设每个 Node 最少需要 30ms 的期待时间，10mb 的存储空间，可想而知，创建几千个 Node 是不太现实的。
...
以下这些方法是**同步**的，意味着他会**阻塞**事件循环，暂停执行代码，直到spawned的进程退出。
...
`spawnSync`直到子进程关闭的时候才会返回.超时或者收到`killSignal`信号时，也不会返回，直到进程完全退出。进程处理`SIGTERM` 信号后并不会结束，直到子进程完全推出。
...
* `options` {Object}
  ...
  * `maxBuffer` {Number}
  * `encoding` {String} stdio 输入和输出的编码方式. (默认: 'buffer')
* return: {Buffer|String} 来自命令的 stdout 

直到子进程完全退出，`execFileSync` 才会返回。超时或者收到`killSignal`信号时，也不会返回，直到进程完全退出。进程处理`SIGTERM` 信号后并不会结束，直到子进程完全推出。
...
直到子进程完全退出，`execSync` 才会返回。超时或者收到`killSignal`信号时，也不会返回，直到进程完全退出。进程处理`SIGTERM` 信号后并不会结束，直到子进程完全推出。
