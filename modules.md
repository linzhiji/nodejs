...
Node 有简单的模块加载系统。在 Node 里，文件和模块是一一对应的。下面例子里，`foo.js` 和  加载同一个文件夹里的 `circle.js` 模块。
...
加载到模块里的变量是私有的，仿佛模块包含在一个函数里。在这个例子里， `PI` 是  `circle.js` 的私有变量。
...
当环形调用 `require()`是，当返回时模块可能都不会执行结束
...
`main.js`加载完两个模块时，它们都结束。这个程序的输出如下： 
...
如果按照文件名没有找到，那么 node 会试着加载添加了后缀`.js`, `.json`的文件，如果还没好到，再试着加载添加了后缀`.node` 的文件。
...
文件前没有 `/` 或 `./` 前缀，表示模块可能是 `core module`，或者已经从`node_modules` 文件夹里加载了。
...
如果没有找到，那么会到父目录，直到到文件系统的根目录。

例如，如果 `'/home/ry/projects/foo.js'` 里的文件 加载 `require('bar.js')`，那么 node 将会按照下面的顺序查找
...
多次调用 `require('foo')` 也许不会导致模块代码多次执行。这是很重要的特性。因此，可以返回 "partially done" 对象，允许加载过渡性的依赖关系，及时会引起环形嗲用

如果你希望一个模块多次调用，那么就输出一个函数，然后调用这个函数。
...
`module.exports` 是由模块系统创建的。有时候感觉这很难接受，很多人希望自己的模块是某个类的实例。因此，把将要导出的对象赋值给  `module.exports`。注意，将想要的对象赋值给 `exports` ，只是简单的将它绑定到本地 `exports`  变量，这可能并不是你想要的。
...
#### exports alias
 ...
 `exports` 变量在 引用到 `module.exports` 的模块里可用。和其他变量一样，如果你给他赋一个新的值，它不再指向老的值。
...
* `id` {String}
* Return: {Object} 已经解析模块的`module.exports` 
...
为了获取即将用 `require()` 加载的准确文件名，使用 `require.resolve()` 函数。
...
## 从全局文件夹加载Loading from the global folders

<!-- type=misc -->

如果环境变量 `NODE_PATH` 被设置为冒号分割的绝对路径列表，如果在模块在其他地方没有找到，node 将会搜索这些路径。（注意，Windows 里，`NODE_PATH`用分号分割 ）。
...
当 node 运行一个文件时， `require.main` 就会被设置为它的 `module`。也就是说你可以通过测试判断文件是否被直接运行。  
...
假设我们希望有一个文件夹`/usr/lib/node/<some-package>/<some-version>`，包含指定版本的包内容。

包可以依赖于其他包。为了安装包 foo，可能需要安装特定版本的 `bar` 包。 `bar`  包可能有自己的包依赖，某些条件下，依赖关系可能会发生冲突或形成循环。
...
因此，及时存在循环依赖或依赖冲突，每个模块还可以获得他所依赖的包得可用版本。
